...
module.exports = function (app) {
  ...
  plugin.registerWithRouter = function (router) {
    ...
    router.get('/getEP', (req, res) => {
      res.json({latitude: 50, longitude: 5})
	  vesselPosition = app.getSelfPath('navigation.position.value')
	  calculateCurrent(vesselPosition)
  ...
  function calculateCurrent (vesselPosition) {
	  // First determine which of the diamonds we're going to use. Store then in chosenDiamonds.
	  chosenDistance = 0
	  chosenDiamonds = []
	  nearestDiamond = []
	  ...
	  for (i=0; i<2; i++) {
		...
		diamonds.forEach(function (diamond, index) {
			distanceToDiamond = calc_distance (vesselPosition.latitude, vesselPosition.longitude, diamond.position.latitude, diamond.position.longitude);
			app.debug(i, vesselPosition, diamond.position, diamond.reference, distanceToDiamond, index)
			if (distanceToDiamond < minimalDistance && distanceToDiamond > chosenDistance) {
				nearestDiamond = index
				minimalDistance = distanceToDiamond
			}
		})
		chosenDiamonds.push(nearestDiamond)
		chosenDistance = minimalDistance
	  }
	  
	  chosenDiamonds.forEach(function (i, index) {
		  app.debug(diamonds[i].reference)
	  })
	  
	  // Then, for each of these diamonds, determine the hours relative to HW of the reference station
